...
#else
#ifdef RGGAME
static void v2X_16( SDL_Surface *src_surface, SDL_Surface *dst_surface );
static void v2dot4X_16( SDL_Surface *src_surface, SDL_Surface *dst_surface );
#else
...
{
#if defined(TARGET_GP2X) || defined(TARGET_DINGUX)
	{ 320,			 200,			 NULL,	   v2dot4X_16, NULL,"Scale2.4x" },
#if defined(TARGET_GCW0)
	{ 320,           200,            shw_8,    shw_16,     shw_32,     "Scale HW"},
#endif
	{ 320,           200,            NULL,     v2X_16,  NULL,  "Scale2x" },
...
#ifdef RGGAME
void v2X_16( SDL_Surface *src_surface, SDL_Surface *dst_surface )
{
	Uint8 *src = src_surface->pixels, *src_temp,
	      *dst = dst_surface->pixels, *dst_temp;
	int src_pitch = src_surface->pitch,
	    dst_pitch = dst_surface->pitch;
	// const int dst_Bpp = 2;         // dst_surface->format->BytesPerPixel
	const int dst_Bpp = dst_surface->format->BytesPerPixel;
...
#ifdef VGA_CENTERED
	// size_t blank = (vga_width*(480-400)/2)*2;//(dst_surface->h - src_surface->h) / 2 * dst_surface->pitch;
	size_t blank = (dst_surface->h - src_surface->h) / 2 * dst_surface->pitch;
	memset(dst, 0, blank);
	dst += blank;
#endif
...
void v2dot4X_16( SDL_Surface *src_surface, SDL_Surface *dst_surface )
{
	Uint8 *src = src_surface->pixels, *src_temp,
	      *dst = dst_surface->pixels, *dst_temp;
	int src_pitch = src_surface->pitch,
